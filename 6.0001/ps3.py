...
def get_word_score(word, n):
    ...
    if word_length != 0:
        for letter in lower_word:
            score_comp_1 += SCRABBLE_LETTER_VALUES[letter]
    
    score_comp_2 = 7 * word_length - 3 * (n - word_length)
...
def deal_hand(n):
    ...
    vowels_minus_asterisk = VOWELS[0:5]

    for i in range(1, num_vowels):
        x = random.choice(vowels_minus_asterisk)
...
def is_valid_word(word, hand, word_list):
    ...
    word_in_list = False
    wildcard = lower_word.find('*')
    if wildcard != -1:
        for i in range(0, 4):
            possible_word = lower_word.replace('*', VOWELS[i])
            if possible_word in word_list:
                word_in_list = True
                break
    else:
        word_in_list = lower_word in word_list
    ...
    return word_in_list and word_in_hand
    #pass  # TO DO... Remove this line when you implement this function
    ...
    while len(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(user_input, hand, word_list):
                ...
                score += word_score
                print("\"" + user_input + "\" earned " + str(word_score) + " points. Total: " + str(score) + " points")
